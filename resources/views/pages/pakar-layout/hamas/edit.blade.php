...
<div id="layoutSidenav">
    ...
    <div class="container-fluid px-4 mt-5">
        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                <form action="{{ route('hama.update', ['id' =>$hama->id]) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="form-group">
                        <label for="exampleFormControlInput1">Nama Hama</label>
                        <input name="nama" type="text" class="form-control w-50" value="{{ $hama->nama_hama }}" id="exampleFormControlInput1" placeholder="Nama">
                    </div>
                    <div class="form-group">
                        <label for="exampleFormControlTextarea1">Deskripsi Hama</label>
                        <textarea name="deskripsi" class="form-control" id="exampleFormControlTextarea1" rows="5" placeholder="Deskripsi">{{ $hama->deskripsi_hama }}</textarea>
                    </div>
                    <div class="form-group">
                        <label for="exampleFormControlTextarea1">Solusi Hama</label>
                        <textarea name="solusi" class="form-control" id="exampleFormControlTextarea1" rows="5" placeholder="Solusi">{{ $hama->solusi_hama }}</textarea>
                    </div>
                    <div class="form-group">
                        ...
                        <select class="form-select" name="obat" aria-label="Default select example">
                            ...
                            @foreach ( $obat as $o )
                            <option value="{{ $o->id }}" @if ($hama->kode_obat == $o->id)
                                selected
